...
{
    [ScopedRegistration]
    public class MenuRoleRepository : RepositoryBase<MASTER_MENU_ROLE>, IMenuRoleRepository
    {
        public DbDataContext _db;
        public DbReadDataContext _dbRead;

        public MenuRoleRepository(DbDataContext db, DbReadDataContext DbRead) : base(db, DbRead)
        {
            _db = db;
            _dbRead = DbRead;
        }
