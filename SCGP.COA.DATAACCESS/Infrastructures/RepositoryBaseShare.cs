...
{
    public class RepositoryBaseShare<TEntity> : IDisposable, IRepositoryBaseShare<TEntity> where TEntity : class
    {
        public DbContext _db;
        public DbContext _dbRead;

        public RepositoryBaseShare(DbContext db, DbContext dbRead)
        {
            _db = db;
            _dbRead = dbRead;
        }
        ...
        {
            var dd = _dbRead.Database.GetDbConnection().ConnectionString;
            return _dbRead.Set<TEntity>();
        }
