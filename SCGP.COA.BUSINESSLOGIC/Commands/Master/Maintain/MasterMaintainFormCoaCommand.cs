...
namespace SCGP.COA.BUSINESSLOGIC.Commands.Master
{
    ...
    public class MasterMaintainFormCoaCommand : IMasterMaintainFormCoaCommand
    {
        private IMasterMaintainFormCoaRepository _masterRepository;
        public MasterMaintainFormCoaCommand(IMasterMaintainFormCoaRepository masterRepository)
        {
            _masterRepository = masterRepository;
        }

        public SearchResModel<MasterMaintainFormCoaSearchResultModel> SearchData()
        {
            var query = _masterRepository.SearchData();

            var res = new SearchResModel<MasterMaintainFormCoaSearchResultModel>()
            {
                PageIndex = 1,
                PageSize = 10
            };
        ...
        public MasterMaintainFormCoaModel CreateData(MasterMaintainFormCoaModel req)
        {
            var res = _masterRepository.CreateData(req);
            return res;
        }

        public MasterMaintainFormCoaModel GetData(int req)
        {
            var res = _masterRepository.GetData(req);
            return res;
        }

        public MasterMaintainFormCoaModel UpdateData(MasterMaintainFormCoaModel req)
        {
            var res = _masterRepository.UpdateData(req);
            return res;
        }
        public MasterMaintainFormCoaModel DeleteData(int req)
        {
